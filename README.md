...
`live:doctor`, `live:prepare`, and `live:stack` accept `--feed-port`, `--app-port`, `--archive-dir`, `--db`, `--clips`, `--qa-dir`, and `--kick-tunnel-check` overrides. Use these instead of hand-editing commands when a port, evidence path, clip queue export path, QA artifact folder, or tunnel proof path needs to change right before recording.
