...
The submission bundle command writes `evidence-report.txt`, `replay.json`, `replay.csv`, `submission-notes.md`, and `summary.json` into one folder for the final handoff. If `qa/live-run-plan.txt`, `qa/final-report.md`, or `qa/final-report.json` exists, it also copies them into the bundle. The notes file gives a human-readable proof summary, repo commit, and external artifact checklist, while the JSON summary keeps platform counts, source labels, throughput, latency metrics, run-sheet paths, and repo metadata from the recorded run.
