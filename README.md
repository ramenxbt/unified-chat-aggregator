...
The submission bundle command writes `evidence-report.txt`, `replay.json`, `replay.csv`, `submission-notes.md`, and `summary.json` into one folder for the final handoff. If `qa/evidence-check.txt`, `qa/live-run-plan.txt`, `qa/final-readiness.txt`, `qa/final-report.md`, `qa/final-report.json`, `qa/obs/obs-browser-sources.*`, `qa/visual/manifest.*`, or `qa/kick-tunnel-check.txt` exists, it also copies them into the bundle. Pass `--clips clip-queue.json` after exporting the dashboard clip queue to copy and summarize the marked submission moments as `clip-queue.json`. Strict final bundles reject missing or non-ready saved evidence proof, stale final QA reports, missing or stale final-readiness proof, partial run sheets, saved run sheets from a different commit, run sheets missing launch or evidence commands, OBS handoff files from a different commit, missing or malformed OBS handoff files, stale or malformed visual QA manifest files, missing or stale Kick tunnel proof, missing or malformed clip queue exports when `--clips` is provided, and OBS handoff URLs that do not match the run sheet, so regenerate `qa/evidence-check.txt`, `qa/live-run-plan.txt`, `qa/final-readiness.txt`, `qa/obs/`, `qa/visual/`, and `qa/kick-tunnel-check.txt` after the final commit, port change, proof-threshold change, or evidence path change. The notes file gives a human-readable proof summary, repo commit, clip queue summary, and external artifact checklist, while the JSON summary keeps platform counts, source labels, throughput, latency metrics, evidence-check proof path, run-sheet paths, final-readiness proof path, OBS handoff paths, visual QA manifest paths, Kick tunnel proof path, clip queue path, and repo metadata from the recorded run.
