...
`live:doctor`, `live:prepare`, and `live:stack` accept `--feed-port`, `--app-port`, `--archive-dir`, `--db`, `--clips`, and `--kick-tunnel-check` overrides. Use these instead of hand-editing commands when a port, evidence path, clip queue export path, or tunnel proof path needs to change right before recording.
