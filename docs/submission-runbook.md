...
npm run live:ready -- --out qa/final-readiness.txt
...
The saved `qa/live-run-plan.txt` file keeps the exact commands, target source labels, OBS URLs, tunnel health check, evidence commands, post-recording finalizer command, clip-aware bundle command, replay export commands, and repo commit metadata available during setup. Set `TWITCH_BROADCASTER_LOGIN` and `KICK_BROADCASTER_SLUG` even when you already have numeric IDs so the final overlay can prove account-qualified labels like `TWITCH (MARKETBUBBLE)` and `KICK (MARKETBUBBLE)`.
...
The final `live:ready` gate checks a clean tracked worktree, strict connector preflight, three readable target source labels, usable final QA evidence, usable visual QA manifest evidence, current strict run sheet with the launch commands, OBS all-source URL for the current app port, and current evidence commands, valid current-commit OBS handoff files, and matching OBS all-source URLs between the run sheet and handoff before OBS setup. It accepts an older `qa/final-report.json` when Git shows no final-QA-relevant files changed since that report commit and the current cheap repo hygiene scan still passes. It accepts an older `qa/visual/manifest.*` when Git shows no UI-relevant files changed since that manifest commit, so skip fresh screenshots after server-only, docs-only, or runbook-only changes. It also prints the saved Kick tunnel proof command to run after the capture stack starts and the exact `npm run live:stack -- --require-ready --with-proof-gate` command for the final capture. Save it with `--out qa/final-readiness.txt` so the go/no-go result sits beside the run sheet. If connector setup is incomplete, it repeats the strict preflight details and `.env` checklist inline.
...
If a default port, evidence path, clip queue path, final artifact folder, tunnel proof path, or proof wait window is unavailable, pass the same overrides to `live:doctor`, `live:prepare`, `live:ready`, and `live:stack`, for example `--feed-port 8899 --app-port 5260 --archive-dir data/final-sessions --db data/final.sqlite --clips exports/final-clips.json --evidence-check qa/evidence-check.txt --kick-tunnel-check qa/kick-tunnel-check.txt --proof-timeout-ms 300000`. Use `--obs-handoff-dir` and `--visual-qa-dir` on `live:ready` and `live:stack` when those final artifacts live outside `qa/obs` or `qa/visual`.
...
Do not treat partial mode as final proof. It is only for connector smoke testing. If you use the printed proof, evidence, finalizer, or bundle commands during a partial rehearsal, keep the included `--allow-partial` flag.
Before the final strict bundle, regenerate `qa/live-run-plan.txt` without `--allow-partial`, save `qa/final-readiness.txt`, rerun `npm run obs:handoff -- --out qa/obs`, save `qa/evidence-check.txt`, and save `qa/kick-tunnel-check.txt` from the running capture stack; `submission:bundle` will fail with an artifact issue if current tracked files are dirty, if the saved evidence proof is missing or non-ready, if the final QA report is missing, malformed, failed, dirty, stale after final-QA-relevant changes, or older while current repo hygiene fails, if the final readiness proof is missing or stale, if the run sheet is partial, stale, or missing launch/evidence commands, if the OBS handoff files are stale, missing, or malformed, if the visual QA manifest is malformed, if the visual QA manifest is stale after UI-relevant changes, if the Kick tunnel proof is missing or stale, or if the OBS handoff URL does not match the run sheet. Rerun `npm run qa:final` after source, script, e2e, config, or dependency changes. Rerun `npm run qa:visual` only after UI-relevant changes such as dashboard source, styling, fixtures, protocol, route, dependency, or visual capture script changes.
...
If the proof gate waits on signal, use the saved run sheet's `Proof signal checklist`: confirm a Twitch chat event from the target channel, a Kick chat event through the public tunnel, and an X post or Space result matching `X_FILTER_RULES` or `X_SPACES_QUERY` before starting the final recording.
...
- Visual QA screenshots from `qa/visual/`, current or older with no UI-relevant changes since capture.
- Visual QA manifest from `qa/visual/manifest.md`, current or older with no UI-relevant changes since capture.
